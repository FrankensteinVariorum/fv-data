...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xsl:variable name="xSpacer" select="700"/>
   <xsl:variable name="max_Xval" select="$countColls * $xSpacer"/>
    <xsl:template match="/">
        <svg width="2000" height="2000" viewBox="0 0 2000 {$max_Yval * 4.5}">
            <g transform="translate(50 7000)" >
      ...
      <xsl:for-each select="$spineColl">
          ...
          <line x1="{position() * $xSpacer}" y1="0" x2="{position() * $xSpacer}" y2="-{//app/@n ! number() => max()}" stroke="red" stroke-width="20"/>
